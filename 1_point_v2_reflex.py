...
def cost_function(params, *args):
    E_sam, E_ref_w, E_sam_w, freqs, n_PVA, k_PVA, n_PLA, k_PLA, theta_input = args
    ...
    n_s = [TDSC.n_air, n_PLA, n_PVA, n_PLA]
    k_s = [0, k_PLA, k_PVA, k_PLA]
    ...
    H_teo = H_T3.H_sim_rouard_ref(freqs, n_s, k_s, thick_s, theta_input)  # theta in air = 45 deg
    ...
    # # quit()
    # print(1e-7 * np.sum(delta_phi) ** 2)
    # quit()
    return np.sum(delta_E**2) + 1e-3 * np.sum(delta_abs**2) + 1e-7 * np.sum(delta_phi**2)
...
ref_file = './data/reflex/ref' + str(sample_num) + '.txt'
...
print('PLA =', H_T3.theta_from_air(theta_input, 1.62) * 180 / np.pi)
print('PVA =', H_T3.theta_from_air(theta_input, 1.89) * 180 / np.pi)
quit()
...
A_constraint = spy_opt.LinearConstraint(A_constraint, (280e-6), (320e-6))  # , -20e-6), (320e-6, 20e-6))
...
if __name__ == '__main__':
    t_sam, E_sam = rd.read_1file('./data/reflex/sam' + str(sample_num) + '.txt')
    ...
    for num_stats in range(num_statistics):
        ...
        #                        np.array((70e-6, 160e-6, 70e-6)),  # , 1, 1, 1, 1)),
